...
> **当前进度（截至 0.1.0，2026-08-12）**：P0 工程化门槛、P1 可插拔深化、P2 生产可用
...
> （Provider 超时 + 结构化输出强制）已完成，216 测试全绿。
...
| 测试 | 15 个测试文件、216 用例全绿（外部服务全 mock），`gearlink/core` 覆盖率 95.80% | `tests/` | ✅ |
...
## 4. P1：可插拔深化 ✅ 已完成（0.2.x 主线，已并入 0.1.0）
...
## 5. P2：生产可用 ✅ 已完成（0.3.x 主线，已并入 0.1.0）
...
| M1 | 0.2.x（已并入 0.1.0） | P1 全部 | ✅ 已完成 | 四类扩展点各 ≥ 2 个实现；核心接口零破坏性变更；`pytest` 全绿且新增用例覆盖新参数默认值等价现状 |
| M2 | 0.3.x（已并入 0.1.0） | P2 全部 | ✅ 已完成 | 会话可持久化恢复；事件可落盘回放；多 Agent 示例可运行；`ModelResponse.usage` 生效 |
| M3 | 0.2.0 | §6.4 + §6.5（前两项） | ✅ 已完成 | 架构治理（去全局单例 + 统一记忆契约）落地；Provider 超时 + 结构化输出强制；216 测试全绿 |
