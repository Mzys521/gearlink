...
- **token 计数可注入** ✅：`TokenCounter` 协议统一文本、消息和预算内截断，
  `ShortTermMemory` / `MemoryManager` / `ReactAgent` / `PlanExecuteAgent` 均支持注入；
  默认沿用零依赖启发式，可选 `TiktokenTokenCounter`；工具结果与截断标记共同受
  `MAX_TOOL_RESULT_TOKENS` 硬上限约束，消除按字符反推对中文、代码和 emoji 的误差；
...
| M4 | 后续版本 | P3 继续 | ⏳ 未排期 | CLI 可日常使用；剩余 §6.5–6.7 逐步落地 |
