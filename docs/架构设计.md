...
flowchart TB
    ...
    subgraph runtime["核心运行时"]
        ...
        TOKENS["TokenCounter<br/>计数与预算内截断"]
    end
    ...
    AGENT -->|"tool result budget"| TOKENS
...
classDiagram
    ...
    ReactAgent --> TokenCounter : 工具结果预算
    ...
    ShortTermMemory --> TokenCounter : 滑窗预算
    MemoryManager --> TokenCounter : 上下文 / 检索 / 压缩预算
    TokenCounter <|.. HeuristicTokenCounter
    TokenCounter <|.. TiktokenTokenCounter
...
| `TokenCounter` | `utils/token_count.py` | 统一文本/消息计数与预算内截断；默认 `HeuristicTokenCounter` 零依赖，可选 `TiktokenTokenCounter` 或第三方协议实现；供 Agent 工具结果和 Memory 各级预算共同注入 | 不等同于 Provider 返回的实际 `TokenUsage` |
...
- `token_counter`：`ShortTermMemory` 的滑窗预算与 `MemoryManager` 的压缩阈值、检索配额、总上下文和 system 配额统一依赖 `TokenCounter`；默认启发式保持历史行为，可注入 tiktoken 或模型专用实现。现有“至少保留最新消息/system/首条检索”的软约束不变。
...
└── utils/                   # 通用辅助与全局日志开关
    ...
    ├── token_count.py       # TokenCounter 协议、启发式/tiktoken 实现、预算内截断
...
| `token_counter_demo.py` | TokenCounter 注入 / 预算内截断 / 可选 tiktoken 精确计数 | 是 |
