...
flowchart TB
    ...
    subgraph core[核心层 core/]
        AGENT["Agent 抽象<br/>ReactAgent / PlanExecuteAgent（core/agent.py）"]
        ORCH["Orchestrator 多 Agent 协作（core/orchestrator.py）"]
        EVT["AgentEvent 事件体系 + JSONL 落盘回放（core/events.py）"]
        MEM["Memory 抽象<br/>ShortTermMemory / LongTermMemory / MemoryManager / VectorStore（core/memory.py）"]
        DISP["工具调度<br/>ToolRegistry（注册表 + schema + 调度器，core/tool.py）"]
    end

    subgraph ext[扩展层]
        PROV["providers/<br/>OpenAIProvider / OllamaProvider / AnthropicProvider"]
        TOOLS["tools/<br/>builtin.py / load_skill.py 内置工具"]
        SKILLS["skills/<br/>Skill / SkillRegistry / SkillLoader"]
        MCPX["mcp/<br/>McpClient 外部 MCP 工具接入"]
    end

    EXC["gearlink/exceptions.py<br/>GearLinkError 异常体系（横切所有层）"]
    UTIL["gearlink/utils/<br/>logging / token_count / usage（横切辅助）"]
    ...
    ORCH --> AGENT
    ORCH -.-> EVT
    TOOLS -.->|"register_tool 注册进"| DISP
    MCPX -.->|"映射为 mcp_<server>_<tool> 注册进"| DISP
    SKILLS -.->|"组合系统提示 + load_skill 工具"| AGENT
```

**依赖方向铁律**：箭头只允许从外向内。扩展层实现核心层定义的抽象，通过构造函数注入（如 `ReactAgent(provider=OpenAIProvider())`）；核心层永远不反向 import 扩展层的具体实现。唯一的例外是 `core/` 对 `providers/base.py` 与 `skills/base.py` 中契约类型的依赖，属于对抽象的依赖。可直接运行的演示代码一律放在根目录 `examples/`，不得内嵌于 `core/` 源文件。
...
| `Orchestrator` | `core/orchestrator.py` | 多 Agent 协作编排（主管-工人）：主管（纯 LLM，无工具）把任务拆分为子任务并指派给登记的工人（各为独立 `Agent`，可配独立工具/记忆/技能），各工人完成后由汇总器整合为最终答案；分派解析失败退化为全员兜底；`parallel=True` 时工人并行执行（事件按序产出）。与 `PlanExecuteAgent`（单模型串行步骤）互补 | 不重复实现 ReAct 循环（复用 `Agent` 作为主管与工人） |
| `DependentOrchestrator` | `core/orchestrator.py` | 依赖编排（`Orchestrator` 子类）：构造时编程式声明工人依赖（`dependencies`），执行按 worker 级依赖做 Kahn 拓扑分层（层内可并行、层间串行），并把上游工人结果以 `[上游结果]` 报告段落自动注入下游任务，实现流水线式协作；构造时校验未登记引用与依赖环（抛 `GearLinkError`）；`dependencies=None` 时行为等价 `Orchestrator` | 不修改现有 `Orchestrator` 行为 |
| `AutonomousOrchestrator` | `core/orchestrator.py` | 模型自主编排（`Orchestrator` 子类）：主管每次请求自主产出 DAG 计划（`{"nodes": ..., "edges": ...}`，节点为子任务、边为数据依赖），校验后编译为节点级 Kahn 拓扑分层混合串并行执行（层内并行、层间串行），并按依赖边聚合全部直接上游结果注入下游任务；计划非法（解析失败/未登记工人/缺失依赖/环）时记录日志并降级为全员兜底分派 | 不修改现有 `Orchestrator` / `DependentOrchestrator` 行为 |
...
```
README.md（项目简介 + 快速开始 + 公共 API 一览）
    ↓
docs/使用教程.md（新手完整教程：安装 → 基础 → 进阶）
    ↓
docs/架构设计.md（本文档：分层、组件、数据流、扩展点契约）
    ↓
docs/接口文档.md（每个公共 API 的作用与用法说明）
    ↓
docs/开发规范.md（贡献者必读）
    ↓
docs/开发方向.md（P0~P3 路线图与里程碑）
```
