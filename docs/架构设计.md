...
- **`Orchestrator` 额外事件**：`TeamPlanGeneratedEvent`（主管分派清单；`DependentOrchestrator` 额外携带 `dependencies` 字段）→ 每个子任务 `AgentHandoffEvent`（派单，依赖编排时 `task` 为注入后文本）→ 工人事件（转发并重新编号）→ `SubtaskEndEvent`；多子任务且 stream=True 时汇总文本以 `TextDeltaEvent` 产出。
