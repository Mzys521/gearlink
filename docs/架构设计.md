...
| 组件 | 位置 | 职责 | 不负责 |
|---|---|---|---|
| `ReactAgent` | `core/agent.py` | 编排 Reason→Act→Observe 循环；管理轮数上限 `MAX_ITERATIONS`；工具失败作为可恢复信号写回记忆 | 不感知具体模型 SDK、不实现具体工具 |
...
### 5.4 Memory（已落地短期/长期，组合器待实现）

继承 `Memory`，实现 `add_message / get_messages / clear` 三方法。`LongTermMemory` 额外提供 `get_relevant_messages()` 语义检索。

## 6. 记忆架构目标形态

`MemoryManager`（现为空类）的目标职责——短期管「当下对话」，长期管「沉淀知识」：

```mermaid
flowchart LR
    AGENT[ReactAgent] -->|"add_message"| MM[MemoryManager]
    MM --> STM["ShortTermMemory<br/>滑窗 N 条，每轮全量注入"]
    MM -->|"选择性沉淀"| LTM["LongTermMemory<br/>chromadb 向量库"]
    AGENT -->|"build_context(query)"| MM
    MM -->|"get_relevant_messages(query, k)"| LTM
    LTM -.->|"top-k MemoryEntry 转 system 提示注入"| AGENT
```

建议接口（实现时补齐 docstring 与测试）：

```python
class MemoryManager:
    """记忆管理器：组合短期与长期记忆，统一为 Agent 提供上下文"""

    def __init__(self, short_term: Memory, long_term: LongTermMemory | None = None) -> None: ...
    def add_message(self, message: dict[str, Any]) -> None:
        """写入短期记忆；按策略（如会话结束钩子）沉淀到长期记忆。"""

    def build_context(self, query: str, relevant_limit: int = 5) -> list[dict[str, Any]]:
        """组装本轮请求消息：长期记忆 top-k 检索结果注入 + 短期记忆全量。"""
```
...
| 4 | 实现 `MemoryManager` 并在步骤 3 的注入点上替换默认记忆 | 纯新增；`Memory` 抽象签名不变 |
