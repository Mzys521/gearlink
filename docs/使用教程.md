...
默认计数器无第三方依赖，旧工具函数继续可用：

```python
from gearlink import count_message_tokens, estimate_tokens, truncate_text

estimate_tokens("这是一段待估算的文本")
count_message_tokens({"role": "user", "content": "你好"})
truncate_text("很长的文本" * 100, 20, suffix="...[截断]")
```

需要模型对应的精确 BPE 计数时，安装 `gearlink[tokenizers]` 并把同一计数器注入
Agent 与 Memory：

```python
from gearlink import MemoryManager, ReactAgent, ShortTermMemory, TiktokenTokenCounter

counter = TiktokenTokenCounter(model="gpt-4o")
short_term = ShortTermMemory(max_tokens=4_000, token_counter=counter)
memory = MemoryManager(
    short_term=short_term,
    max_context_tokens=8_000,
    token_counter=counter,
)
agent = ReactAgent(provider=provider, memory=memory, token_counter=counter)
```

`ReactAgent` 用计数器限制工具结果，截断提示也在 `MAX_TOOL_RESULT_TOKENS` 预算内；
`ShortTermMemory` / `MemoryManager` 分别用它完成滑窗、压缩阈值、检索和上下文配额。
对于非 OpenAI 模型，应传入兼容的 `encoding_name`，或自行实现 `TokenCounter` 协议。
并行工具调用时，自定义计数器必须线程安全。
...
| [token_counter_demo.py](../examples/token_counter_demo.py) | 可注入 token 计数 / 预算内截断 / 可选 tiktoken | 是 |
...
truncate_text("长文本", 100, token_counter=counter, suffix="...[截断]")  # 后缀计入预算
TiktokenTokenCounter(model="gpt-4o")  # 需安装 gearlink[tokenizers]
```
